...
int main() {
    ...
    pointToPoint.SetDeviceAttribute("DataRate", StringValue("50000bps"));  //500bps 減少頻寬以模擬雍塞
    pointToPoint.SetChannelAttribute("Delay", StringValue("5ms"));     // 增加延遲

    NetDeviceContainer devices;
    devices = pointToPoint.Install(nodes);
    
   // 創建一個 Priority Queue
    Ptr<QueueDisc> queue = CreateObject<PrioQueueDisc>();
    ...
    tch.SetRootQueueDisc("ns3::PrioQueueDisc");
     ...
     // 設定不同優先權的封包
    Ptr<PrioQueueDisc> prioQueue = DynamicCast<PrioQueueDisc>(queue);
    ...
    if (prioQueue) {
        for (int i = 1; i <= 10; ++i) {
            uint32_t priority = 10-i;  // 設定優先權
            uint32_t band = i;
            prioQueue->SetBandForPriority(priority, band);
            uint32_t packetSize = packet_count[i - 1]; 
...
//-----------------------------------------------------------------------------------------------------------------------------------
    ...
    std::vector<FlowMonitor::FlowStats> flowStatsContainer(11);
